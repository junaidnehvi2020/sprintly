...
export default function UserManagementPage() {
  ...
  const { resources, roles: projectRoles, applications, isLoading: isDataLoading } = useData();
  ...
  const combinedUsers = useMemo<CombinedUser[]>(() => {
    return authUsers.map(user => {
      ...
      return {
        ...
        projectApplication: application?.name || '-',
      };
    });
  ...
  const uniqueProjectRoles = useMemo(() => [...new Set(projectRoles.map(r => r.name))], [projectRoles]);
  const uniqueApplications = useMemo(() => [...new Set(applications.map(a => a.name))], [applications]);
  const accessRoleOptions = ['Admin', 'Manager', 'Member', 'Guest'];
  ...
  return (
    <Card>
      ...
      <CardContent>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-6 p-4 border rounded-lg bg-muted/50">
